...
Orchestrates the development workflow by understanding the domain context, mapping changes to the correct architecture layer, and implementing with consistent patterns. Uses discovered project conventions from `/developer discover`.
...
- **Setup**: `/developer discover` (run once during framework setup)
...
|------|---------|---------|
| **discover** | `/developer discover` | Analyze project architecture and patterns |
...
# Generated by: /developer discover
...
**Requires:** `.claude/skills/developer.yaml` exists (run discover first)
